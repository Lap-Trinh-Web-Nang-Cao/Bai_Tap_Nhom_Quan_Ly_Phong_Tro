...
var SupportPage = (function ($) {
            ...
            columnDefs: [
                ...
                },
                {
                    ...
                    render: function (data, type, row) {
                        ...
                        html += '<button class="btn btn-sm btn-info" onclick="SupportPage.viewDetail(\'' + id + '\')" title="Xem chi tiết"><i class="fa fa-eye"></i></button>';
                        if (row.TrangThai === 'Moi') {
                            html += '<button class="btn btn-sm btn-warning" onclick="SupportPage.process(\'' + id + '\')" title="Bắt đầu xử lý"><i class="fa fa-play"></i></button>';
                        }
                        if (row.TrangThai === 'Moi' || row.TrangThai === 'DangXuLy') {
                            html += '<button class="btn btn-sm btn-success" onclick="SupportPage.complete(\'' + id + '\')" title="Hoàn thành"><i class="fa fa-check"></i></button>';
                            html += '<button class="btn btn-sm btn-danger" onclick="SupportPage.reject(\'' + id + '\')" title="Từ chối"><i class="fa fa-times"></i></button>';
                        }
                        html += '</div>';
                        return html;
                    }
    ...
    function initFilters() {
        $('#searchInput').on('keyup', function (e) {
            if (e.keyCode === 13) table.ajax.reload();
        });
    ...
    function process(id) {
        ...
        swal({
            ...
            $.post(config.urls.process, { id: id })
                .done(function (res) {
                    if (res.success) swal("Thành công", res.message, "success").then(function () { table.ajax.reload(); });
