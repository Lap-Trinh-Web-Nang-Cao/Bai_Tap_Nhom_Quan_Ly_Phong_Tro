...
namespace USER_QUANLYPHONGTRO.Services
{
    ...
    public class AuthService
    {
        ...
        public AuthService()
        {
            _apiBaseUrl = (ConfigurationManager.AppSettings["ApiBaseUrl"] ?? "http://localhost:7039").TrimEnd('/');
        ...
        public async Task<LoginResponse> LoginAsync(string email, string password)
        {
            try
            {
                ...
                var apiBase = (ConfigurationManager.AppSettings["ApiBaseUrl"] ?? "http://localhost:7039").TrimEnd('/');
